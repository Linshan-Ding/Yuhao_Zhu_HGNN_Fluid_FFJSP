...
from agent.ppo import update as ppo_update, categorical_kl
...
def preference_update(policy,optimizer,replay,c,rng,version):
    ...
    for _ in range(sc['replay_batches']):
        ...
        before=deepcopy(policy.state_dict());opt=deepcopy(optimizer.state_dict())
        optimizer.zero_grad();loss.backward();gn=torch.nn.utils.clip_grad_norm_(policy.parameters(),c['training']['max_grad_norm']);optimizer.step()
        with torch.no_grad():kl=float(categorical_kl(old,policy(batch).logp,batch.offsets))
        if not np.isfinite(kl) or kl>sc['kl']:
            policy.load_state_dict(before);optimizer.load_state_dict(opt);rejected+=1;break
        metrics.append((float(loss.detach()),kl,float(gn)))
