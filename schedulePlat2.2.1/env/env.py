...
class SchedulingEnv(gym.Env):
    ...
    def reset(self, mbom_df, orders_df, machines_df, current_time):
        ...
        self.orders_df = copy.deepcopy(orders_df)  # 深拷贝订单 DataFrame
        self.machines_df = copy.deepcopy(machines_df)  # 深拷贝机器 DataFrame
        self.mbom_df = copy.deepcopy(mbom_df)  # 深拷贝物料清单 DataFrame
    ...
    def _get_obs(self) -> np.ndarray:
        """
        构造观察向量：将机器状态矩阵和工序状态矩阵拼接后展平为一维数组
        """
        mach = self.state.feat_mas.numpy().flatten()
        task = self.state.feat_opes.numpy().flatten()
        obs = np.concatenate([mach, task])
    ...
    def step(self, action):
        ...
        # 如果当前时间加上加工时间超过订单交期，则需要丢弃该订单# 如果该工序为订单的最后一个工序，则更新订单状态为完成
        if self.current_time + proc_time > order_due_time:
            ...
            self.discard_order_ids.append(selected_order_id)
        ...
        # 计算奖励: 相邻决策点丢弃订单数的差值
        reward_discard = self.order_discard_count_last - self.order_discard_count
