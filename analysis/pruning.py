...
oracle 只有一种实现，即 `lookahead_oracle`：逐个候选动作执行一步，再用固定参考策略
做 N 次公共随机数 rollout，取均值最高者，并同时报告 rollout 标准误。它本身是估计量，
因此保留率必须与其标准误一起报告，不能只报点值。（早先文档提到的"小档对残余问题
精确求解"的 oracle 从未实现，论文 §5.7 的相应表述已删去。）
"""
